...
class Client(object):
    def __init__(self,args,id,train_samples,test_samples):
        ...
        if self.noise_type == 'human':
            self.dataset_path = os.path.join(self.data_dir, self.dataset,
                                             self.noise_type + "_" + self.partition + "_" + balance_str + "_" + str(
                                                 self.num_clients))
        ...
        self.local_noise_ratio = get_dataloder(self.train_path,self.id, self.batch_size,True).dataset.noise_ratio
    ...
    def save_result(self):
        ...
        while len(self.train_loss) < len(self.test_acc):
            self.train_loss.append(-404)
        while len(self.test_acc) < len(self.train_loss):
            self.test_acc.append(-404)

        result_df = pd.DataFrame({'train_loss': self.train_loss, 'test_acc': self.test_acc})
        result_df.to_csv(os.path.join(result_path, str(self.id) + ".csv"), index=False)
