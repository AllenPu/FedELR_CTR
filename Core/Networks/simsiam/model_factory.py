...
from .resnet_cifar import ResNet18, ResNet34, ResNet50, ResNet101, ResNet152, ResNet18_1c
import torch
...
class SimSiam(nn.Module):
    def __init__(self,output_channel, emam, args):
        super(SimSiam, self).__init__()
        self.emam = emam

        if args.dataset in ['mnist', 'fmnist']:
            self.backbone = SimSiam.get_backbone('resnet18_1c')
            self.backbone_k = SimSiam.get_backbone('resnet18_1c')
        else:
        ...
        self.linear = Linear(nb_classes=output_channel, feat=dim_in)
