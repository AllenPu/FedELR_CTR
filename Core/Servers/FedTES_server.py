...
from Core.Clients.FedTES_client import FedTESClient
import logging
import sys
...
class FedTES(Server):
    ...
    def train(self):
        ...
        if self.tensorboard:
            ...
            writer_local_acc = SummaryWriter(os.path.join(tensorboard_path, 'local_acc'))
            writer_after_aggregate = SummaryWriter(os.path.join(tensorboard_path, 'after_aggregate'))
        for i in range(self.global_rounds + 1):
            ...
            if i == 0:
                ...
                # id, total_acc, total_test_num,averaged_acc = self.local_eval()
                averaged_acc = self.global_eval()
                for client_id in id:
                    writer_after_aggregate.add_scalar('Client' + str(client_id) + 'noise' + str(self.clients[client_id].local_noise_ratio), averaged_acc, i - 1)
            ...
            for client_id in id:
                writer_local_acc.add_scalar('Client' + str(client_id) + 'noise' + str(self.clients[client_id].local_noise_ratio), total_acc[client_id], i)
    ...
    def local_eval(self):
        ...
        for client in self.clients:
            ...
            test_acc.append(acc / test_num)

        averaged_acc = sum(total_acc) * 1.0 / sum(total_test_num)

        return id, test_acc, total_test_num, averaged_acc
